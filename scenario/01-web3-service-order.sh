...
# Standalone showcase: the optional Base Sepolia live smoke report is looked
# up under the example root. Operators who produce one in a separate run
# can either copy it into this path or pass --require-base-sepolia-smoke
# with an explicit artifact-dir to fail-closed when it is absent.
if [[ -f "${EXAMPLE_ROOT}/target/web3-live-rollout/base-sepolia/base-sepolia-smoke.json" ]]; then
  REQUIRE_BASE_SEPOLIA=1
fi
