...
def _chio_bin() -> str:
    override = os.environ.get("CHIO_BIN")
    if override:
        return override
    resolved = shutil.which("chio")
    if resolved:
        return resolved
    raise RuntimeError(
        "chio binary not found. Install via https://www.chio.world/install.sh "
        "or set CHIO_BIN to an absolute path."
    )


def _run_chio(args: list[str], *, cwd: Path = EXAMPLE_ROOT) -> Json:
